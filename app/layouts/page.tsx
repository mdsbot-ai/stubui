  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '48px 24px' }}>
        <h1 style={{ margin: '0 0 8px', fontSize: '22px', fontWeight: 'bold' }}>Layout Templates</h1>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '48px' }}>
          {layouts.map((layout) => (
            <div key={layout.slug}>
              <h2 style={{ margin: '0 0 4px', fontSize: '14px', fontWeight: 'bold' }}>{layout.name}</h2>
              <p style={{ margin: '0 0 16px', fontSize: '12px', color: 'var(--muted)' }}>{layout.description}</p>
